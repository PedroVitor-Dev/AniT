...
public sealed record LibraryScanProgress(int FilesProcessed, int TotalFiles, string CurrentFile);
...
public sealed class LibraryScanner(AniTDbContext database)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var files = await Task.Run(() => Directory.EnumerateFiles(root.Path, "*", SearchOption.AllDirectories)
        ...
        foreach (var path in files)
        {
            ...
            {
                processed++;
                progress?.Report(new LibraryScanProgress(processed, files.Count, Path.GetFileName(path)));
            }
        }
